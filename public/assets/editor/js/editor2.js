...
(function( $ ){
	var methods = {
				...
				class:"row-fluid"
					...
					id:"tblForm"+idExtn
						...
						class:"row"
							...
							class:"col-xs-12 col-sm-6 col-md-6 col-lg-6"
							}).append($('<label/>',{ for:"tblAlign"+idExtn, text:"Alineamiento"}
							)).append($('<select/>',{ id:"tblAlign"+idExtn, class:"form-control form-control-width"}
								).append($('<option/>',{ text:"Seleccionar", value:""}
								)).append($('<option/>',{ text:"Izquierda", value:"left"}
								)).append($('<option/>',{ text:"Centro", value:"center"}
								)).append($('<option/>',{ text:"Derecha",	value:"right"}))
							).append($('<label/>',{	for:"tblBorder"+idExtn, text:"Grosor de Borde"}
							)).append($('<input/>',{
								id:"tblBorder"+idExtn,
								type:"text",
								class:"form-control form-control-width",
								value:1
							})).append($('<label/>',{ for:"tblCellspacing"+idExtn,	text:"Espacios"}
							)).append($('<input/>',{
								id:"tblCellspacing"+idExtn,
								type:"text",
								class:"form-control form-control-width",
								value:1
							})).append($('<label/>',{ for:"tblCellpadding"+idExtn,	text:"Padding"}
							)).append($('<input/>',{
		...
		init : function( options )
		{
			...
			var menuItems = { 'fonteffects': true,
						   ...
						   'insert_img'	: { "modal": true,
											...
											"onSave": function(){
												...
												if($('#imageList').data('current')){
													...
													methods.showMessage.apply(this,["imgErrMsg","SEleccionar Una Imagen"]);
													return false;
												}
						...
						'insert_table'	: { "modal": true,
						   ...
						   'outdent'	: { "text": "Anular sangría",
							...
							'select_all': { "text": "Selecionar Todo",
			...
			/*Aca Puedo manipular el menu que se muestra en el editor de texto*/
			var settings = $.extend({
				'texteffects':true,
				'aligneffects':true,
				'textformats':true,
								            	...
								            	class:"modal-dialog"
							            			...
							            			class:"modal-content"
									            		...
									            		class:"modal-footer"
								                			...
								                			type:"button",
								                			class:"btn btn-success",
								               				}).html('Crear').mousedown(function(e){
								                			e.preventDefault();
